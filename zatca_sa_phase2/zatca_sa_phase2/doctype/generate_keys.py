...
import requests
import base64
from utils import get_fatoora_base_url
...
def generatekeys(config_dict=config_dict):
    ...
    subprocess.run([os.path.join(openssl_dir, 'openssl'), 'req', '-new', '-sha256', '-key', 'PrivateKey.pem', '-extensions', 'v3_req',
                    '-config', get_config(config_dict=config_dict), '-out', 'taxpayer.csr'])
