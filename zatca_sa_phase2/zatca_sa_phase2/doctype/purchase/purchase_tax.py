...
def tax_Data(invoice,p_invoice_doc):
            try:
                ...
                cbc_TaxAmount.set("currencyID", p_invoice_doc.currency) # SAR is as zatca requires tax amount in SAR
                tax_amount_without_retention =  round(abs(get_tax_total_from_items(p_invoice_doc)),2)
                ...
                cbc_TaxAmount_2.set("currencyID", p_invoice_doc.currency)
                ###
                # cbc_TaxAmount_2.text = str(tax_amount_without_retention) # str(abs(sales_invoice_doc.base_total_taxes_and_charges))
                cbc_TaxAmount_2.text = str(round( sales_json['taxes'][0]['tax_amount'] ,2))# str(abs(sales_invoice_doc.base_total_taxes_and_charges))
                ...
                if p_invoice_doc.custom_zatca_tax_category == "Standard":
                    cbc_ID_8.text = "S"
                elif p_invoice_doc.custom_zatca_tax_category == "Zero Rated":
                    cbc_ID_8.text = "Z"
                elif p_invoice_doc.custom_zatca_tax_category == "Exempted":
                    cbc_ID_8.text = "E"
                elif p_invoice_doc.custom_zatca_tax_category == "Services outside scope of tax / Not subject to VAT":
                    cbc_ID_8.text = "O"

                cbc_Percent_1 = ET.SubElement(cac_TaxCategory_1, "cbc:Percent")
                # cbc_Percent_1.text = str(sales_invoice_doc.taxes[0].rate)
                cbc_Percent_1.text = f"{float(p_invoice_doc.taxes[0].rate):.2f}" 
                ...
                cbc_LineExtensionAmount.set("currencyID", p_invoice_doc.currency)
                cbc_LineExtensionAmount.text =  str(abs(p_invoice_doc.base_total))
                cbc_TaxExclusiveAmount = ET.SubElement(cac_LegalMonetaryTotal, "cbc:TaxExclusiveAmount")
                cbc_TaxExclusiveAmount.set("currencyID", p_invoice_doc.currency)
                cbc_TaxExclusiveAmount.text =str(abs(p_invoice_doc.base_total-p_invoice_doc.base_discount_amount))
                cbc_TaxInclusiveAmount = ET.SubElement(cac_LegalMonetaryTotal, "cbc:TaxInclusiveAmount")
                cbc_TaxInclusiveAmount.set("currencyID", p_invoice_doc.currency)
                cbc_TaxInclusiveAmount.text = str(round(abs(p_invoice_doc.base_total) + abs(sales_json['taxes'][0]['tax_amount'])-abs(sales_invoice_doc.base_discount_amount),2))
                cbc_AllowanceTotalAmount = ET.SubElement(cac_LegalMonetaryTotal, "cbc:AllowanceTotalAmount")
                cbc_AllowanceTotalAmount.set("currencyID", p_invoice_doc.currency)
                # sdict = sales_invoice_doc.as_dict()
                # discount_amount = sdict['discount_amount']               
                cbc_AllowanceTotalAmount.text = str(round(p_invoice_doc.discount_amount,2))
                ...
                cbc_PayableAmount.set("currencyID", p_invoice_doc.currency)
                cbc_PayableAmount.text = str(round(abs(p_invoice_doc.base_total) + abs(sales_json['taxes'][0]['tax_amount'])  - abs(p_invoice_doc.base_discount_amount) ,2))
                return invoice
             
            except Exception as e:
                        frappe.throw("error occured in tax data"+ str(e) )

def tax_Data_with_template(invoice,p_invoice_doc):
            ...
            try:
                ...
                for item in p_invoice_doc.items:
                ...
                cbc_TaxInclusiveAmount = ET.SubElement(cac_LegalMonetaryTotal, "cbc:TaxInclusiveAmount")
                cbc_TaxInclusiveAmount.set("currencyID", p_invoice_doc.currency)
                cbc_TaxInclusiveAmount.text = str(round(abs(p_invoice_doc.base_total) + abs(sales_json['taxes'][0]['tax_amount']) - abs(p_invoice_doc.base_discount_amount),2))
                cbc_AllowanceTotalAmount = ET.SubElement(cac_LegalMonetaryTotal, "cbc:AllowanceTotalAmount")
                cbc_AllowanceTotalAmount.set("currencyID", p_invoice_doc.currency)
