...
def delivery_And_PaymentMeans(invoice,p_invoice_doc, is_return):
            try:
                customer_doc= frappe.get_doc("Supplier",p_invoice_doc.supplier)
                ...
                cbc_ActualDeliveryDate.text = str(p_invoice_doc.due_date)
...
def delivery_And_PaymentMeans_for_Compliance(invoice,p_invoice_doc, compliance_type):
            try:
                cac_Delivery = ET.SubElement(invoice, "cac:Delivery")
                cbc_ActualDeliveryDate = ET.SubElement(cac_Delivery, "cbc:ActualDeliveryDate")
                cbc_ActualDeliveryDate.text = str(p_invoice_doc.due_date)
