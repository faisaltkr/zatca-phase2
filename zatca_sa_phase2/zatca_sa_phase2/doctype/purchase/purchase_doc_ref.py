...
from .get_invoice_hash import get_latest_purchase_invoice_with_hash
...
def doc_Reference(invoice,p_invoice_doc,invoice_number):
            try:
                cbc_DocumentCurrencyCode = ET.SubElement(invoice, "cbc:DocumentCurrencyCode")
                cbc_DocumentCurrencyCode.text = p_invoice_doc.currency
                ...
                cbc_UUID_1.text = str(get_icv_code(p_invoice_doc))
                return invoice  
            except Exception as e:
...
def additional_Reference(invoice,supplier_doc,invoice_number):
            try:
