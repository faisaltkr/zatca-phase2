...
def xml_structuring(invoice,p_invoice_doc):
            try:
                ...
                try:
                    fileX = frappe.get_doc(
                        {   "doctype": "File",        
                            "file_type": "xml",  
                            "file_name":  "E-invoice-" + p_invoice_doc.name + ".xml",
                ...
                try:
                    frappe.db.get_value('File', {'attached_to_name':p_invoice_doc.name, 'attached_to_doctype': p_invoice_doc.doctype}, ['file_name'])
                except Exception as e:
                    frappe.throw(frappe.get_traceback())
            except Exception as e:
...
def extract_certificate_details(customer_doc):
            try:    
                    ...
                    if customer_doc.is_custom_b2c == 1:
