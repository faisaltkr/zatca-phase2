...
def invoice_Typecode_Simplified(invoice,p_invoice_doc):
            try:                             
                cbc_InvoiceTypeCode = ET.SubElement(invoice, "cbc:InvoiceTypeCode")
                if p_invoice_doc.custom_invoice_transaction_type1 in invoice_Typecode_Simplified_dict:
                    type_code = invoice_Typecode_Simplified_dict[p_invoice_doc.custom_invoice_transaction_type1]
                else:
                    type_code = "0200000"
                cbc_InvoiceTypeCode.set("name", type_code) # Simplified

                if p_invoice_doc.is_return == 0 and p_invoice_doc.is_debit_note !=1: 

                    # cbc_InvoiceTypeCode.set("name", type_code) # Simplified
                    cbc_InvoiceTypeCode.text = "388"
                elif p_invoice_doc.is_return == 1:       # return items and simplified invoice
                    # cbc_InvoiceTypeCode.set("name", type_code)  # Simplified,
                    cbc_InvoiceTypeCode.text = "381"  # Credit note
                elif p_invoice_doc.is_debit_note == 1:
...
def invoice_Typecode_Standard(invoice,p_invoice_doc):
            try:
                    # print(sales_invoice_doc.custom_invoice_transaction_type1,"lsdjfdslfjdlflfdjkl")
                    if p_invoice_doc.custom_invoice_transaction_type1 in invoice_Typecode_Standard_dict:
                          type_code = invoice_Typecode_Standard_dict[p_invoice_doc.custom_invoice_transaction_type1]
                    else:
                          type_code = "0100000"
                    cbc_InvoiceTypeCode = ET.SubElement(invoice, "cbc:InvoiceTypeCode")
